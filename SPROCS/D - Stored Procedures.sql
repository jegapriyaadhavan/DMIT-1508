...
SELECT * FROM Position
-- DELETE FROM Position WHERE PositionID = 12
--Try manually inserting a position with a name hat is too long
INSERT INTO Position(PositionDescription)
...
--The position table has declared the PostionDescription as a varchar(50).
--That means the database will not accpect a description that is longer because attempting to do so would "lose" data (the string valuse would be "truncated").
--What's the higest PositionID so far?
